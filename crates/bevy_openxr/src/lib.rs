...
pub fn add_plugins_fn(group: &mut PluginGroupBuilder) -> &mut PluginGroupBuilder {
    group.add_before::<bevy::core::CorePlugin, _>(OpenXRPlugin);
    //group.remove::<bevy_winit::WinitPlugin>();
    group.remove::<RenderPlugin>();
    group.add_after::<bevy::scene::ScenePlugin, _>(get_render_plugin());
...
pub fn get_render_plugin() -> RenderPlugin {
    RenderPlugin {
        base_render_graph_config: Some(bevy::render::render_graph::base::BaseRenderGraphConfig {
            add_2d_camera: false,
            add_3d_camera: true,
            ..Default::default()
        }),
